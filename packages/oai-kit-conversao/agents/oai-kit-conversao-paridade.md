...
- **Paginação/listagem (qualquer padrão com grid), bloqueante**: hook de listagem sempre com `placeholderData: keepPreviousData` (`@tanstack/react-query`) — ausência trava o grid na página antiga até um segundo clique (armadilha #52; já escapou de 14 telas antes de virar checklist, 2026-08-12). Em telas pai-filho/master-detail onde a query tem `enabled` condicionado ao registro-pai selecionado: `keepPreviousData` + `enabled: false` retém dados do registro anterior no cache — usar guard condicional nos dados exibidos, nunca renderizar cache de um pai diferente do selecionado (armadilha #60).
- Se a tela usa `DataGridSearchServer` fora do escopo dos itens acima: comparar as props passadas contra `catalogo-reuso/componentes/DataGridSearchServer.md` — sinalizar como ponto de atenção qualquer prop não coberto pela receita do padrão escolhido; se os dados vierem de array local (não paginado no servidor), confirmar que `rows` é o subset da página atual, nunca o array completo (armadilha #56). Se a tela usa `BrowserDePesquisa`, confirmar que os parâmetros de paginação estão em `paginationModel`, não soltos no topo de `params` (armadilha #54, ver `catalogo-reuso/componentes/BrowserDePesquisa.md`).
- Nenhum `TextField` usa o prop `mask` (crash em runtime confirmado — armadilha #23); campos numéricos/alfanuméricos de tamanho fixo usam `inputProps={{ maxLength: N }}` + regex Zod. Nenhum schema Zod usa `invalid_type_error` (removido no Zod v4, quebra o build com `TS2353` — usar `error`, armadilha #57).
- **Padrões de layout/componente transversais** (grep-detectáveis, bloqueantes): nenhum `Box component="form"` como wrapper de formulário (deve ser `Form` do UIKit, ver `catalogo-reuso/componentes/Form.md`); **nenhum import de qualquer `@mui/*`** (`@mui/material`, `@mui/icons-material`, `@mui/lab`, etc. — tudo deve vir de `@praxio/globusweb-uikit`, ícones de `@praxio/globusweb-uikit/icons`; ver armadilha #14); campo de PK em modo edição nunca usa `disabled` (deve ser `InputProps={{readOnly}}`); toda operação de save/delete tem `LoadingDialog` de overlay junto a `isSaving`/`isDeleting` (nunca só botão desabilitado); pares "campo curto + campo que ocupa o resto" usam `size:"auto"`+`size:"grow"` (nunca fração fixa + `sx={{maxWidth}}`, armadilha #40); comparação de PK numérica vinda de GraphQL sempre com `Number()`/`String()` explícito, nunca `===` estrito (armadilhas #3/#38); `CheckListBox.onChange` sempre tratado como array de objetos (`.map(o => o.value)`), nunca assumido como array de strings (armadilha #55); `autoHeight` do `DataGridSearchServer` nunca combinado com wrapper `Box` de `height: "100% !important"` (gera espaço vazio entre linhas e paginação, armadilha #58); `helperText` inline de `TextField` nunca ao lado de `IconButtonMaterial` no mesmo `Box display="flex"` sem ajuste de alinhamento (quebra `alignItems="center"`, armadilha #59).
- Toda query GraphQL que alimenta um `Combobox`/lookup de busca (ex.: `useFuncionariosOptions` e hooks compartilhados equivalentes) passa `paging`/`limit` explícito e alto — nunca confiar no default do backend (`limit: 10` é comum e trunca silenciosamente a lista, armadilha #49).
- `service.ts` de REST client nunca usa `ConnectionApi.getServiceName(sigla)` quando `GlobusWeb.Config.json` tiver mais de uma entrada com a mesma `key` (ambíguo, resolve pra entrada errada — armadilha #50); nesse caso, usar sempre o literal do `name` real do serviço de backend.
- Se a entidade usa `@UseProximoCodigo()`: confirmar que a coluna PK tem precisão suficiente para o volume esperado — `NUMBER(1)`/`NUMBER(2)` estoura `ORA-01438` rapidamente quando o maior código já está em uso (armadilha #51).
- Coluna Oracle `DATE` usada para armazenar só hora (`TTime` do Delphi) com `LocalDateTimeTransformer`: confirmar evidência de leitura/round-trip via `from()` documentada (ver `cheatsheets/convencoes-implementacao.md`) — aplicar o transformer sem esse cuidado desloca o valor em runtime (mesma causa raiz de PTSTM-007/GAP-002).
