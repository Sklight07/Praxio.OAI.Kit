...
- Nunca aplique `LocalDateTimeTransformer` numa coluna `DATE` que armazena só hora (`TTime` do Delphi) sem confirmar o comportamento de leitura/round-trip via `from()` — aplicado sem esse cuidado, desloca o valor em runtime mesmo com a UI parecendo correta (ver `cheatsheets/convencoes-implementacao.md`, mesma causa raiz de PTSTM-007/GAP-002).
