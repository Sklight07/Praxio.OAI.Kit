...
2. Se cross-módulo, checar `implementacaoBackend` da tabela (grep pelo nome exato no diretório `{knowledgeBasePath}/tabelasConhecidas/` — um arquivo por módulo desde 2026-08-19, ~2700 entradas no total, nunca `Read` de arquivo inteiro — ver AP-CONV-006):
...
1. **Cache primeiro, sempre**: grep pelo nome exato da tabela no diretório `{knowledgeBasePath}/tabelasConhecidas/` (separado do índice desde 2026-08-05, um arquivo por módulo desde 2026-08-19, ~2700 entradas no total — **nunca `Read` de arquivo inteiro** só para checar uma tabela). Se a entrada existir, **o MCP não é chamado para esta tabela** — vá direto ler `{knowledgeBasePath}/<arquivo apontado>` (a entrada guarda só o ponteiro; colunas/tipos/PK/FK completos estão no arquivo `descobertas-oracle/<TABELA>.md`). Vale mesmo se o arquivo tiver notas `[completar]` de divergência Delphi-vs-Oracle pendentes (import em massa sem `.pas` cruzado ainda) — o schema já está confirmado, só falta comparar contra o `.pas` desta tela (passo 5), não rechamar o MCP.
