...
### Adicionado
- **Novo agente `oai-kit-conversao-guardiao`** — gate rígido e mecânico de conformidade de padrão de backend, acionado no novo **PASSO 2.5** de `/oai-kit-converter-tela` (entre Backend e Frontend, incondicional em todos os níveis `N1`-`N-ESPECIAL`). Checklist fechada de 21 itens, cada um PASS/FAIL sem meio-termo — cobre SQL injection, transação manual, repository/service customizado sem justificativa, subscriber duplicado, PK composta como falsa justificativa para resolver manual, `@ResolveField` sem defeito documentado, coleção filha 1:N/N:M via cascade, `@Transactional()` decorando guard clause, `QueryService` desnecessário, entity de outro módulo, `@CreateDateColumn`/`@UpdateDateColumn`, nomenclatura (`@ObjectType`/propriedade/sincronização com `.module.ts`), `AbstractRepository` vs `BaseRepository`, localização de testes, resolver customizado novo, e `ValidationPipe` em Controller REST. Qualquer FAIL sem justificativa documentada no patch bloqueia o avanço ao frontend.
- **`AP-CONV-020`** — abstrações de transação (`@Transactional`)/repositório (`BaseRepository`/`AbstractRepository`)/integração (`IRequestsService`) são default, não opcionais; implementação manual exige justificativa documentada.
- **`AP-CONV-022`** — coleção filha 1:N/N:M com dono único usa `@OneToMany`/`@ManyToOne` + cascade declarado, nunca repository/service/resolver customizado; com duas ressalvas de implementação (`orphanedRowAction:'delete'` pode não bastar sozinho no `updateOne` quando a PK da entity filha é só FK+1 coluna; no `deleteOne` do pai, sempre precisa de `BeforeDeleteOneHook` explícito, independente da PK).
- **`AP-CONV-023`** — nunca usar `@CreateDateColumn()`/`@UpdateDateColumn()` contra uma tabela Oracle legada (não calculam a data em JS para insert de uma linha só, dependem de `DEFAULT SYSDATE` real no banco que uma tabela pré-existente não tem); data de inclusão/alteração é setada explicitamente em `Before*Hook`.
- **`AP-CONV-024`** — `@ObjectType`/propriedade nunca leva prefixo de módulo; abreviação só é expandida por extenso quando o significado é conhecido com certeza (nunca adivinhar); nenhuma propriedade repete o nome da própria entity/DTO; todo rename sincroniza os nomes de operação do `.module.ts` (strings independentes, não derivadas automaticamente).
- **`AP-CONV-025`** — repository de entity própria sem SQL cru usa `BaseRepository<Entity>`; `AbstractRepository` só para SQL cru/múltiplas tabelas/sem entity única — critério não é ownership do módulo (confirmado contra `MaterialRepository`/`CadastroDefeitoRepository` reais em `GlobusWeb.Manutencao`).
- **`AP-CONV-026`** — `{nullable: false}` no DTO/Input substitui validação manual de campo obrigatório puro quando a coluna Oracle já não aceita NULL.
- **`AP-CONV-027`** — testes ficam em `tests/<rotina>/`, nunca ao lado do fonte — confirmado como padrão real já usado em `GlobusWeb.Acidentes`/`GlobusWeb.Manutencao`.

### Alterado
- **`AP-CONV-021` reescrita por completo** — de "ação de negócio customizada vira resolver, nunca REST" para **resolver GraphQL customizado é último recurso absoluto, nunca criado sem aprovação explícita do dev documentada no patch**. Não existe mais categoria de ação (simples ou complexa, leitura ou escrita) que justifique resolver por padrão. Resolvers já existentes no código — inclusive em módulos de referência como Acidentes/Manutencao — nunca são justificativa para criar um novo (são precedente pendente de retrofit). Ordem obrigatória antes de cogitar resolver: CRUD automático (inclusive leitura simples por filtro) → `Before*Hook` → REST Controller+Service+Repository (chamada a sistema externo ou operação atômica multi-step) → resolver, só com aprovação explícita do dev. Validado retroativamente eliminando um resolver real já em produção (`AgrupamentoAreaEmpresaFilialResolver`, GlobusWeb.Folha — 2 queries + 1 mutation, nenhuma sobrou como resolver).
- Hierarquia absoluta de implementação em `oai-kit-conversao-backend.md` reordenada para refletir a nova AP-CONV-021, incluindo o requisito de `@UsePipes(new ValidationPipe({transform:true}))` local em todo Controller REST novo com `@Body()` (confirmado que `GlobusWeb.Folha` não tem `ValidationPipe` global; verifique o module raiz do módulo-alvo antes de assumir o mesmo).
...
### Adicionado
- **Enforcement de 12 armadilhas novas (#49-52, #54-60)** no checklist estático de `oai-kit-conversao-paridade.md` e na lista espelhada de `conversion-policy.md`: paginação/listagem sem `placeholderData: keepPreviousData` (#52) ou com cache indevido em pai-filho (#60); dados locais de `DataGridSearchServer` sem subset de página (#56); `BrowserDePesquisa` com paginação fora de `paginationModel` (#54); Combobox/lookup sem `paging` explícito (#49); `CheckListBox.onChange` tratado como strings (#55); `service.ts` usando `getServiceName` com chave ambígua (#50); `@UseProximoCodigo()` sem checar precisão da coluna (#51); `LocalDateTimeTransformer` sem evidência de round-trip (relacionado a PTSTM-007/GAP-002); Zod `invalid_type_error` removido no v4 (#57); `autoHeight` + `Box height:100%!important` (#58); `helperText` quebrando `alignItems` com `IconButtonMaterial` (#59). Restrições espelhadas em `oai-kit-conversao-frontend.md`/`-backend.md` onde aplicável.
- **GAP-005 corrigido**: `oai-kit-conversao-triagem.md` (passo 2) agora verifica `especificacoes[tela].status === "convertida"` antes de reaproveitar/reprocessar uma tela, e pergunta ao dev antes de prosseguir. `oai-kit-conversao-especificador.md` (passo 7) escreve `status: "documentada"`; `oai-kit-conversao-aprendizado.md` (passo 1) escreve `status: "convertida"` — antes essa convenção só existia nos dados (preenchimento manual), sem nenhum agente instruído a escrevê-la ou checá-la.
- Ressalva da exceção de PK não-digitável (só `onRowDoubleClick`, sem `onBlur`) propagada dos resumos condensados de `oai-kit-conversao-frontend.md`/`-paridade.md` — já estava correta no arquétipo completo, mas os resumos tratavam `onBlur` como universal do Inline+Grid, risco de falso-positivo/implementação forçada.

### Corrigido
- `oai-kit-conversao-aprendizado.md`: regra explícita — `gapsAbertos > 0` sempre implica `resultado: "convertido_com_gaps"`, nunca `"convertido"` (achado: 44% das telas "convertido" numa semana real tinham GAP aberto). `metrics/README.md` (Minerva) documenta o valor `retrabalhoPosPadraoAtualizado`, em uso real mas não catalogado.
- `cheatsheets/delphi-para-nestjs.md` (Minerva): receita de `DATE` hora-pura + `LocalDateTimeTransformer` ganha a ressalva de round-trip já existente em `convencoes-implementacao.md` — evita repetir o bug de timezone que já ocorreu 2 vezes (Horários, corrigido; Indisponíveis, GAP-002 ainda aberto — fix de código pendente de decisão do dev).
- `catalogo-reuso/hooks-e-utils.md`, `catalogo-reuso/componentes/DataGridSearchServer.md`, `archetypes/crud-simples-pk-usuario.md`, `archetypes/crud-simples-pk-gerada.md`, `archetypes/lookup-readonly.md`, `cheatsheets/delphi-para-react.md` (Minerva): propagação das armadilhas #49, #51, #54, #56, #57, #58, #60 pros documentos-fonte onde a receita correspondente é ensinada (antes só existiam no cheatsheet, sem chegar a quem implementa).

**Origem (segunda rodada)**: dev pediu propostas de otimização adicionais após a auditoria. Duas aprovadas para implementação imediata: (1) fechar o loop de propagação na origem, não só via auditoria retroativa; (3) reestruturar o checklist transversal do paridade, que tinha virado um único parágrafo enorme.

### Adicionado (continuação)
- **`cheatsheets/paridade-checklist-transversal.md`** (Minerva, novo): checklist estruturado (não mais prosa corrida) com todas as verificações transversais de `oai-kit-conversao-paridade` — paginação/listagem, combobox/lookup, backend/dados, layout/componente. `oai-kit-conversao-paridade.md` e `conversion-policy.md` passam a referenciar este arquivo em vez de duplicar/acumular o conteúdo cada um por conta própria.
- **Gate mecânico de propagação em `oai-kit-conversao-aprendizado.md`**: toda armadilha nova exige uma decisão explícita e registrada antes do Gate Pré-Commit — grep-detectável vira item no checklist transversal/arquétipo, ou justificativa documentada de por que não. Substitui a instrução anterior ("avalie se é transversal"), que existia mas não bastou sozinha — auditoria encontrou 11 de 12 armadilhas da semana sem essa decisão registrada.
...
### Corrigido
- `oai-kit-conversao-frontend.md`, `oai-kit-conversao-paridade.md` e `conversion-policy.md` (AP-CONV-010): comando de build explicitado como `npm run build:backend`/`npm run build:frontend` a partir da raiz do módulo — nunca `npm run build` puro.

**Origem (segunda rodada)**: dev identificou que o comando `build:frontend` usa `tsc --noEmit` (só typecheck) enquanto o pipeline do Azure builda com `build:iis`/`tsc -b` (build real via project references) — telas que passavam na verificação do agente estavam falhando só no CI do Azure.

### Corrigido (continuação)
- `oai-kit-conversao-frontend.md`, `oai-kit-conversao-paridade.md` e `conversion-policy.md`: comando do frontend trocado de `npm run build:frontend` para `npm run buildiis:frontend` — mesmo `tsc -b` que o Azure roda, evita passar erro que só apareceria no CI. Backend mantido em `build:backend` (já usa `nest build`, sem a mesma divergência).
...
**Revisão pontual (2026-08-07) de `commands/oai-kit-registrar-gap.md`** — o comando só conhecia a categoria GAP; a categoria irmã formalizada nesta sessão (Descarte consciente, `gaps/descartes-log.md`) não tinha caminho de registro fora do fluxo completo de conversão. PASSO 1 vira triagem de 3 vias (nada / GAP / Descarte); PASSO 2/3 adaptados por categoria (Descarte não é indexado, checagem por grep; gate próprio); Restrições Absolutas ganham as regras equivalentes de Descarte.
...
**Revisão pontual (2026-08-07) de `commands/oai-kit-documentar-tela.md`** — o recap do PASSO 1 (o que `oai-kit-conversao-especificador` faz) estava desatualizado: faltavam AP-CONV-016 (LGPD), AP-CONV-017 (combobox de referência), a taxonomia de elemento sem equivalente visual e Descartes conscientes (todos adicionados nesta sessão), além de duas omissões antigas nunca refletidas aqui — detecção cross-módulo (AP-CONV-012) e resolução de menu/índice (AP-CONV-013). Bullet list expandida para cobrir os 6 pontos.

**Revisão pontual (2026-08-07) de `README.md` e `commands/oai-kit-converter-tela.md`** — `README.md` ainda afirmava que as regras de branch/commit ficam "no gate final de paridade" (obsoleto desde a branch-antecipada: a branch nasce na etapa 1b da triagem, paridade só commita); lista de comandos não mencionava que `/oai-kit-registrar-gap` também cobre Descarte; menção a policies parava em "AP-CONV-015+" sem citar AP-CONV-016/017. `commands/oai-kit-converter-tela.md`: mesmo gap do `oai-kit-documentar-tela.md` — o recap do PASSO 1 (Triagem) não mencionava o passo 4f (detectar campo sensível/LGPD e campo de referência/combobox quando não há especificação prévia), justamente o caminho mais comum onde AP-CONV-016/017 podiam passar batido; também faltava a ressalva de que lupa da própria PK não conta como referência externa na pontuação estrutural (AP-CONV-017).
