...
- **Padrões de layout/componente transversais** (2026-08-07, ampliado 2026-08-10): nenhum `Box component="form"` como wrapper de formulário (deve ser `Form` do UIKit); **nenhum import de qualquer `@mui/*`** (`@mui/material`, `@mui/icons-material`, `@mui/lab`, etc. — tudo vem de `@praxio/globusweb-uikit`, ícones de `@praxio/globusweb-uikit/icons`; armadilha #14, regra confirmada pelo dev em 2026-08-06); campo de PK em modo edição nunca usa `disabled` (deve ser `InputProps={{readOnly}}`); toda operação de save/delete tem `LoadingDialog` de overlay junto a `isSaving`/`isDeleting`; pares "campo curto + campo que ocupa o resto" usam `size:"auto"`+`size:"grow"` (armadilha #40); comparação de PK numérica vinda de GraphQL sempre com `Number()`/`String()` explícito, nunca `===` estrito (armadilhas #3/#38).
...
- **Paginação/listagem** (2026-08-14, achado de auditoria — bug real escapou de 14 telas antes de virar checklist): hook de listagem sempre com `placeholderData: keepPreviousData` (armadilha #52); em pai-filho/master-detail com query `enabled` condicional, guard contra cache do registro-pai anterior (armadilha #60); dados locais (não paginados no servidor) em `DataGridSearchServer` sempre com `rows` = subset da página atual (armadilha #56); `BrowserDePesquisa` com paginação em `paginationModel`, nunca solta no topo de `params` (armadilha #54).
- **Combobox/lookup de busca** (2026-08-14): query GraphQL alimentando `Combobox` sempre com `paging`/`limit` explícito e alto — nunca confiar no default do backend, que pode truncar silenciosamente (armadilha #49); `CheckListBox.onChange` tratado como array de objetos, nunca de strings (armadilha #55).
- **`service.ts` REST** (2026-08-14): nunca `ConnectionApi.getServiceName(sigla)` quando houver `key` duplicada em `GlobusWeb.Config.json` — usar o literal do `name` real do serviço (armadilha #50).
- **`@UseProximoCodigo()`** (2026-08-14): coluna PK com precisão suficiente para o volume esperado — `NUMBER(1)`/`NUMBER(2)` estoura `ORA-01438` (armadilha #51).
- **`LocalDateTimeTransformer` em coluna `DATE` hora-pura** (2026-08-14): exige evidência de leitura/round-trip via `from()` documentada (ver `cheatsheets/convencoes-implementacao.md`) — mesma causa raiz de PTSTM-007/GAP-002.
- **Layout/componente adicionais** (2026-08-14): nenhum schema Zod usa `invalid_type_error` (removido no Zod v4, usar `error` — armadilha #57); `autoHeight` do `DataGridSearchServer` nunca com wrapper `Box height:"100% !important"` (armadilha #58); `helperText` inline nunca ao lado de `IconButtonMaterial` sem ajuste de `alignItems` (armadilha #59).
