...
1. **Para cada tabela**, consulte o cache **antes de cogitar o MCP** — mas nunca via `Read` de `minerva-index.json` para isso (ver nota de tamanho abaixo). Desde 2026-08-19, `tabelasConhecidas` é um **diretório** (`{knowledgeBasePath}/tabelasConhecidas/<SIGLA>.json`, um arquivo por `moduloDono`, ~2700 entradas no total) — a consulta é sempre um **`Grep` pelo nome exato da tabela** (ex.: padrão `"FLP_ESTADOCIVIL":`), apontando o `path` para o **diretório inteiro** `tabelasConhecidas/` quando o módulo dono ainda não é conhecido (caso mais comum), ou direto para `tabelasConhecidas/<SIGLA>.json` quando o módulo já foi resolvido (ex.: via `dicionarioModulos.prefixosTabela`, AP-CONV-012/019) — nunca um `Read` de arquivo inteiro em nenhum dos dois casos. Se a entrada existir, **o MCP não é chamado para esta tabela** — leia diretamente `{knowledgeBasePath}/<arquivo apontado pela entrada>` (o `descobertas-oracle/<TABELA>.md`, que já tem colunas/tipos/PK/FK/constraints completos — o índice guarda só o ponteiro, nunca os detalhes). Isso vale mesmo que o arquivo ainda tenha divergências Delphi-vs-Oracle marcadas `[completar]` (import em massa sem cruzamento de `.pas` ainda) — completar essas notas é comparar contra o `.pas` real da tela, **não** chamar o MCP de novo; o schema em si já está confirmado.
2. **Só as tabelas sem entrada em `tabelasConhecidas/`** (ou com entrada genuinamente stale — schema realmente mudou desde `verificadoEm`, não só nota `[completar]` pendente) seguem a sequência de descoberta: tool dedicada → fallback de dicionário (AP-CONV-005) → **perguntar ao dev** se as duas falharem ou o MCP não estiver configurado. **Adicionar uma entrada nova** (ao contrário da consulta) exige abrir **o arquivo do módulo dono** (`tabelasConhecidas/<SIGLA>.json`, nunca outro) para editar — isso é esperado e aceitável, é uma operação pontual e rara por tabela, diferente da consulta de cache, que é frequente (várias tabelas checadas por tela) e por isso nunca deve custar um `Read` do arquivo/diretório inteiro. **Nunca editar o arquivo de um módulo diferente do `moduloDono` da tabela** — é exatamente o que a divisão por arquivo existe para evitar (conflito de merge entre imports de módulos diferentes acontecendo ao mesmo tempo).

**Nota de tamanho (2026-08-05):** `tabelasConhecidas` foi extraído de `minerva-index.json` para `tabelasConhecidas.json` porque chegou a ser 89% do índice (1531 entradas, ~239KB) — a maior parte de um import em massa de dicionário Oracle (FLP/SRH/ESO/FRQ, 2026-07-31), não descoberta incremental por tela. Isso nunca deve ser podado/removido (as entradas serão necessárias conforme mais telas desses módulos forem convertidas) — só deixou de ser carregado por inteiro a cada agente.

**Nota de tamanho (2026-08-19) — arquivo único virou diretório por módulo:** duas semanas depois, `tabelasConhecidas.json` já tinha 2681 entradas (~501KB, mais que o dobro do gatilho de 2026-08-05) por causa de novos imports em massa (BGM, CTR, CGS) feitos por múltiplos devs em paralelo — o próprio time já vinha inserindo "como append de texto puro, sem tocar entradas de outros devs" por instinto, sinal de que o arquivo único já era um ponto de conflito de merge na prática. Dividido em `tabelasConhecidas/<SIGLA>.json` (agrupado pelo campo `moduloDono` de cada entrada, **nunca** por parsing do nome da tabela — há exceções sem `_` separando o prefixo, ex.: `CTBPARAM`/`CTB`). Cada import em massa de um módulo novo passa a tocar só o arquivo daquele módulo. A consulta por `Grep` continua idêntica na prática — só aponta pro diretório em vez do arquivo único quando o módulo ainda não é conhecido.

**Nota de tamanho (2026-08-14) — mesmo remédio aplicado a `especificacoes` e `componentesUikit`:** essas duas seções chegaram a ~65% do índice (28 telas + 92 componentes, ~40KB dos ~62KB totais) e cresciam sem limite junto com o número de telas convertidas — a meta de 600+ telas do Folha projetava `especificacoes` sozinho passando de 400KB. Extraídas para `especificacoes-index.json` e `componentesUikit-index.json` (arquivos separados, mesmo princípio de `tabelasConhecidas.json`): consulta sempre por `Grep` pelo identificador exato (tela ou componente), nunca `Read` do arquivo inteiro; escrita continua sendo operação pontual normal de abrir e editar. `minerva-index.json` ficou ~8KB — as seções que sobraram (`dicionarioModulos`, `menuLegado`, `menuGlobusWeb`, `gapsAbertos`, `arquetipos`, `padroesFrontend`) são limitadas pelo número de módulos/arquétipos, não pelo número de telas, então não devem repetir este problema.

Depois das duas extrações acima, `minerva-index.json` é genuinamente pequeno (~8KB, era ~850KB no disco antes de 2026-08-05, a maior parte formatação verbosa do PowerShell `ConvertTo-Json` sem nenhuma informação a mais) — seguro para `Read` completo, como o resto dos agentes já assume.
...
**Nota (2026-08-07): a branch é criada no início do fluxo, não mais no gate final.** Até 2026-08-07, a branch só era criada no gate final de `oai-kit-conversao-paridade`, depois do dev testar e aprovar — a implementação inteira acontecia até ali na branch em que o dev estivesse posicionado (tipicamente `develop`). A partir de agora, `oai-kit-conversao-triagem` (etapa 1b, antes de qualquer classificação) sincroniza `develop` do repositório GlobusWeb-alvo e já cria/faz checkout da branch nova — toda a implementação (backend, frontend) acontece **nela**, nunca mais em `develop`. O gate final de `oai-kit-conversao-paridade` não cria mais branch nenhuma (já existe) — só aplica o commit.

**Nome da branch** (resolvido na etapa 1b da triagem, mesma regra de sempre): `feature/{SIGLA}_{SIM|PSE}_{numero}`; se a Task do Azure não tiver SIM/PSE vinculado (nem nela, nem navegando a hierarquia até a Feature/Epic), use o número da própria Task do Azure no lugar: `feature/{SIGLA}_TASK_{ID_AZURE}` (ou `hotfix/...`, conforme a origem).

**Nunca commite diretamente em `develop`/`master`/`main`** em nenhum momento do fluxo. Após o commit final (gate de `oai-kit-conversao-paridade`), o push da branch para o remoto e a retroalimentação do `GlobusEvo.Minerva` (via `oai-kit-conversao-aprendizado`) nunca ficam implícitos — ver gate final em `oai-kit-conversao-paridade`. (Origem: 2026-08-03, reforço pedido após feedback pós-conversão real.)

**Regra absoluta (2026-08-07): nunca fazer merge desta branch de volta para `develop`/`master`/`main`, em nenhuma circunstância.** Nenhum agente de conversão abre PR nem faz merge — isso é sempre decisão e ação do dev, fora do escopo dos agentes. A branch permanece separada até lá; nada do fluxo de conversão jamais escreve em `develop`/`master`/`main` diretamente, nem por commit nem por merge.
...
Antes de usar qualquer componente de `@praxio/globusweb-uikit` numa tela, `oai-kit-conversao-frontend` e `oai-kit-conversao-especificador` consultam **primeiro** `{knowledgeBasePath}/catalogo-reuso/componentes/<Componente>.md` (índice rápido: `componentesUikit-index.json`, arquivo separado de `minerva-index.json` desde 2026-08-14 — consultado sempre por `Grep` pelo nome exato do componente, nunca `Read` do arquivo inteiro, mesmo princípio do AP-CONV-006). Nunca abrem `node_modules/@praxio/globusweb-uikit/dist/*.d.ts` nem a `ui-generator-kb.json` interna do próprio UIKit (tem erros confirmados — props de componentes internos aparecem atribuídas ao componente errado) como primeira parada.

Só cai para leitura de `node_modules`/fonte real (`src/types/*.d.ts` do `GlobusWeb.UIKit`, se o dev tiver o repo local, senão `.d.ts` compilado mesmo) quando o componente **não está catalogado ainda** em `componentesUikit-index.json`. Nesse caso, ao final da conversão, propor a nova entrada via `oai-kit-conversao-aprendizado` seguindo `_template-componente.md` — mesmo fechamento de loop já usado para arquétipos novos (AP-CONV-009 tem o precedente).
...
**A mesma regra vale para primitivas de baixo nível com sufixo `Material`** que o UIKit expõe (`AccordionMaterial`/`AccordionSummaryMaterial`/`AccordionDetailsMaterial`, `TextFieldMaterial`, `IconButtonMaterial`, etc. — wrappers finos sobre o MUI, importados de `@praxio/globusweb-uikit`, não confundir com import direto de `@mui/*`, que é proibido separadamente, armadilha #14). Antes de usar qualquer uma dessas para **montar um padrão do zero** (accordion, formulário, dialog, etc.), consultar `catalogo-reuso/componentes/` por um **componente composto** já pronto para esse padrão exato — ex.: `AccordionGroup` encapsula as três primitivas de accordion; usá-las soltas para reimplementar o que `AccordionGroup` já resolve é a mesma violação de não consultar o catálogo primeiro (armadilha #68, origem: conversão FLP #618284, 2026-08-19). Só usar a primitiva diretamente quando genuinamente não existir componente composto para o padrão — e, nesse caso, é candidato a nova entrada no catálogo, não uma decisão espontânea e não documentada do agente de frontend.
...
**Leitura vs. escrita — dois caminhos válidos, nunca um terceiro:**
- **Leitura**: Federation declarativa (DTO externo `models/external/*.ts` com `@extends`/`@key`/`@external`, referenciado via `@Reference(...)` no DTO de retorno) — o Gateway compõe os campos, zero orquestração manual.
- **Escrita**: `IRequestsService` (`@praxio/globusweb-requests`) chamando a mutation do módulo **dono** da tabela — nunca `repository.save()`/`.update()` local numa entity que mapeia a tabela do outro módulo, mesmo que o schema Oracle físico permita o INSERT/UPDATE direto. Mapear a tabela como `@Entity()` local **e gravar nela** é sempre violação, sem exceção documentável — ver armadilha #95 (Minerva), achado real: `CadastroUsuarioEntity`/`CTR_CADASTRODEUSUARIOS` em `GlobusWeb.Acidentes` (`GAP-010`), um módulo tratado como referência madura que ainda assim cometeu esse erro — nenhum módulo está imune, sempre confira.
- **Exceção reconhecida, não é violação**: JOIN SQL raw somente-leitura contra tabela de outro módulo (sem entity TypeORM mapeando-a) é prática estabelecida no ecossistema GlobusWeb — não force Federation para isso.
...
1. **Dependência já implementada** (`implementacaoBackend.existe: true` na entrada da tabela em `{knowledgeBasePath}/tabelasConhecidas/<SIGLA>.json` — diretório separado do índice desde 2026-08-05, um arquivo por módulo desde 2026-08-19, consultado por grep pelo nome exato da tabela no diretório inteiro, nunca `Read` de arquivo inteiro) — consumir via Federation, documentar a referência (entidade/GraphQL type do módulo dono). **Não força `N-ESPECIAL`** — é tratada como qualquer "referência externa" normal da pontuação estrutural (0/+1/+2). Consumir algo já pronto e documentado é barato.
...
Ao final, `oai-kit-conversao-aprendizado` persiste a implementação nova (ou confirmada) em `implementacaoBackend` da entrada da tabela em `tabelasConhecidas/<SIGLA>.json` (arquivo do módulo dono, nunca outro) e qualquer mapeamento de prefixo↔sigla confirmado com o dev em `dicionarioModulos.prefixosTabela` (esse sim em `minerva-index.json`) — para o próximo módulo que precisar da mesma tabela nunca mais perguntar ou explorar.
...
**Nota (2026-08-18) — mecanismo novo para tela cross-módulo (`EmbeddedScreenModal`), gap do AP-CONV-012/013 ainda aberto:** nem o AP-CONV-012 (domínio de **dado** cross-módulo, via Federation) nem o AP-CONV-013 (resolução de **menu**) cobrem o caso de uma tela/componente de **UI** compartilhado por mais de um módulo, sem menu próprio (gap estrutural identificado analisando `Flp_ordem.pas`, registrado em `GlobusEvo.Minerva/gaps/proposta-seletor-funcionarios-generico.md` — ainda não fechado por este AP-CONV). O UIKit `0.5.584+` passou a oferecer `EmbeddedScreenModal` (ver `catalogo-reuso/componentes/EmbeddedScreenModal.md`): um módulo abre, embutida em iframe com SSO, uma tela inteira que vive em outro módulo. É a ferramenta a avaliar quando o cenário for "abrir a tela inteira de outro módulo" — não resolve o caso de "reaproveitar um componente/filtro" sem tela própria (esse continua sendo cópia por módulo, caso a caso).
...
**Incidente real de violação (2026-09-02, PR #35377, `feature/FLP_TASK_618593`, GlobusWeb.Folha)**: mesmo com esta política já reescrita e mergeada na branch, o agente de backend criou `reintegracao-funcionario.resolver.ts` (3 queries + 1 mutation com validação de negócio) e registrou no patch "resolver manual aprovado" citando o veredito do próprio `oai-kit-architecture-agent` ("APROVADO COM RESSALVAS") como justificativa — nenhuma resposta do dev está registrada em nenhum lugar do plano ou do patch, incluindo uma seção explícita de "Pré-checkpoint: questão ao dev" que ficou sem resposta registrada. **O que conta como aprovação do dev é uma citação verificável de uma pessoa** (trecho literal de mensagem/chat, comentário no PR ou na task do Azure DevOps, com quem e quando) — **nunca** o veredito de outro agente de IA (Guardião ou Arquitetura) escrevendo "aprovado" como parte do seu próprio julgamento técnico automático. Ausência de resposta registrada = FAIL, nunca presumir consentimento por silêncio.
...
**Ressalva de implementação (2026-08-29, armadilha #96)**: `orphanedRowAction: 'delete'` sozinho pode não bastar no `updateOne` quando a PK da entity filha é só as colunas do FK + no máximo 1 coluna extra própria (ex.: `FLP_UNIDADEENTREGAVT_DESTINO`, PK `CODUNID`+`CODAREA`) — o TypeORM pode tentar `UPDATE ... SET FK = NULL` em vez de `DELETE`, e isso falha com `ORA-01407` (a FK também é PK, nunca pode ser NULL). Nesse caso, adicione um `BeforeUpdateOneHook` que apaga explicitamente os itens antigos (`repository.delete({fk: valor})`) antes do update — o INSERT de itens novos via cascade continua automático. Checklist do `oai-kit-conversao-guardiao`, item 13.

**Ressalva adicional — DELETE do pai (mesma data/armadilha)**: no `deleteOne`, o problema é mais amplo e **independe da forma da PK** — `deleteOne` nunca carrega a relação filha antes de excluir a linha pai, então não há cascade de aplicação em nenhum caso, e a constraint Oracle real tipicamente não tem `ON DELETE CASCADE` (o `onDelete: 'CASCADE'` do decorator só afeta DDL de `synchronize`, nunca rodado neste ecossistema). Falha com `ORA-02292` (child record found). Sempre adicione um `BeforeDeleteOneHook` que apaga explicitamente cada coleção filha (`repository.delete({fk: valor})`) antes do delete do pai — mesmo quando a PK da entity filha tem colunas extras suficientes para dispensar o hook no update. Checklist do `oai-kit-conversao-guardiao`, item 14.

### AP-CONV-023 — Nunca usar `@CreateDateColumn()`/`@UpdateDateColumn()` contra uma tabela Oracle legada
...
- **Todo o checklist transversal** (paginação/listagem, combobox/lookup, backend/dados, layout/componente — `mask`, `@mui/*`, `Form`, `LoadingDialog`, `size:"auto"/"grow"`, PK numérica, Zod, etc.) está em `{knowledgeBasePath}/cheatsheets/paridade-checklist-transversal.md` (Minerva) — lido por inteiro por `oai-kit-conversao-paridade`, todo item lá é bloqueante independente de padrão/nível. Não duplicado aqui (extraído em 2026-08-14 de um parágrafo único que crescia a cada armadilha nova).
