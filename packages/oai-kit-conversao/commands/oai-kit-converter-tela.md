...
- **Consulta `especificacoes-index.json` primeiro** (arquivo separado de `minerva-index.json` desde 2026-08-14, grep pelo identificador da tela) — se já existir uma especificação prévia para esta tela (feita via `/oai-kit-documentar-tela`), verifica `status`/staleness e reaproveita conforme o nível registrado (ver abaixo), sem precisar localizar/ler o fonte Delphi inteiro.
