...
**Flag opcional `--com-cypress`** (combinável com qualquer modo acima, ex.: `/oai-kit-documentar-tela {ID_AZURE} --com-cypress`): **por padrão, o esboço de casos de teste não é gerado** — só passando esta flag `oai-kit-conversao-especificador` preenche a seção "Casos de teste" (passo 3e), independente de `/oai-kit-converter-tela` rodar ou não o passo de Cypress depois.
...
- Esboça casos de teste inferidos do Delphi (golden path, regras Tipo 2 já especificadas, validações Tipo 1) — sinalizados como não-exaustivos; **só executado se `--com-cypress` foi passado** (por padrão, este passo é pulado).
...
- Gera `especificacoes/<modulo>/<tela-slug>.md` e atualiza `especificacoes-index.json`.
