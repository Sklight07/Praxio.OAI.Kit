...
2. **ID da User Story (US)** → buscar no Azure os detalhes, critérios de aceite e descrição via `mcp__azure-devops__wit_get_work_item`.
   ...
   **Lidar com anexos da US:** Após buscar a task, verifique se há anexos. Se houver, tente acessá-los via MCP. Se **não** conseguir (qualquer motivo):

   ```
   ⚠️ ANEXO NÃO ACESSÍVEL — aguardando ação do usuário
   ═══════════════════════════════════════════
   Encontrei o(s) seguinte(s) anexo(s) na US #<ID> que não foi(ram) possível(is) ler:
     • <nome do arquivo>
...
1. Confirmar Suite de destino com o QA.

2. Exibir prévia completa do que será criado no Azure antes de qualquer ação:

```
═══════════════════════════════════════════
PRÉVIA — Test Cases a criar no Azure
═══════════════════════════════════════════
Suite: [nome da suite]
Test Plan: [nome do plan]

Test Cases que serão criados:
  • [CTxx.xx] — [título]
  • [CTxx.xx] — [título]
  • [CTxx.xx] — [título]
  (total: X test cases)
...
⚡ **PARADA OBRIGATÓRIA — Não crie nada sem resposta explícita.**

Pergunte: *"Os Test Cases estão corretos? Posso criar no Azure? (sim/não)"*

Só crie após resposta explícita de confirmação. Se o QA ajustar algum CT → revise e confirme novamente.

3. Criar cada CT como Test Case individual via MCP
4. Preencher Steps em XML (`Microsoft.VSTS.TCM.Steps`)
5. Exibir lista dos Test Cases criados com links diretos
6. Se atualização: informar quais foram adicionados e quais substituídos
